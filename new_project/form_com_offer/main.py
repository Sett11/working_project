...
def mock_get_filtered_aircons(area, wifi, inverter, x1, x2, x3, brand):
...
def save_client_data(a,b,c,d):
    return ''

def create_kp_pdf(a,b):
    return ''

def generate_kp(name, phone, mail, area, wifi, inverter, x1, x2, x3, brand):
    """Основная функция: сбор данных, подбор кондиционеров, генерация КП."""
    # 1. Сохраняем данные клиента в БД (пример)
    client_id = save_client_data(name, phone, mail, area)
    
    # 2. Подбираем кондиционеры по параметрам (заглушка)
    aircons = mock_get_filtered_aircons(area, wifi, inverter, x1, x2, x3, brand)
...
with gr.Blocks(title="Автоматизация продаж кондиционеров") as app:
    ...
    generate_btn.click(
        fn=generate_kp,
        inputs=[name, phone, mail, area, wifi, inverter, x1, x2, x3, brand],
